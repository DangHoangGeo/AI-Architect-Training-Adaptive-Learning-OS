...
Twelve real-world, enterprise-grade systems used as reference case studies for architecture practice. Each one models a system that exists in production at real companies, sized and constrained the way an enterprise architect would actually encounter it — not a toy CRUD app.
...
| 12 | [Autonomous Agent Sandbox Platform](12-autonomous-agent-sandbox-platform/README.md) | Developer tools / AI | Per-session microVM isolation and network egress control for agents with free code execution |

## How to use this with the training system

These examples are reference material, not part of the 12-week roadmap sequence. A learner can be pointed at the folder matching the current week's topic for extra worked examples (e.g., Week 4 — Data & Storage → pair with Core Banking Ledger or ERP Integration Hub; Week 9 — AI System Design → pair with Enterprise AI Agent Platform or Autonomous Agent Sandbox Platform; Week 2 — Networking & Secure Boundaries → pair with Autonomous Agent Sandbox Platform; Week 10 — Multi-Tenant SaaS → pair with Multi-Tenant SaaS CRM). They are not required reading and do not gate workshop access.
